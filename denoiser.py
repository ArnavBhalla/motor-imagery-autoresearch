"""
Experiment 16: Score guidance + bigger model (channels=128, depth=6).

Best result so far: score guidance v1/v2 at 4.93% improvement (RMSE 0.034962).
Not_trivial threshold on evaluate() set: RMSE < 0.034670 (6.3% improvement).
Gap: 0.000292 RMSE — one good training run away.

Bigger model (channels=128, depth=6) learns a richer manifold representation,
producing better score predictions and thus more accurate iterative corrections.
Step_size=0.20, n_steps=20 keeps latency ~35-40ms (limit 50ms).
"""
...
class DenoiseNet(nn.Module):
    def __init__(self, traj_dim=TRAJ_DIM, channels=128, depth=6, t_dim=64):
        super().__init__()
...
class DDPM:
    def __init__(self, T_diff=100, t_eval=35, n_steps=20, step_size=0.20,
                 channels=128, depth=6, lr=1e-4, batch_size=128, t_dim=64, device=None):
...
def build_denoiser() -> DDPM:
    return DDPM(T_diff=100, t_eval=35, n_steps=20, step_size=0.20,
                channels=128, depth=6, lr=1e-4, batch_size=128, t_dim=64)
...
if __name__ == "__main__":
    ...
    print(f"Training big score-guidance denoiser for {TIME_BUDGET}s on {denoiser.device} ...")
    ...
    _log_results(results, "score guidance big model ch=128 d=6 n_steps=20 step_size=0.20", git_hash)
