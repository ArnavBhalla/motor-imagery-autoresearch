"""
Experiment 15: Score guidance — fast deterministic version.

Score guidance v2 achieved 4.93% improvement but 204ms latency (limit 50ms).
The fix: n_avg=1 (single fixed-seed draw per step, deterministic), n_steps=12,
step_size=0.45. Expected latency ~16ms (12 steps × ~1.3ms/call).

The larger step_size compensates for fewer steps. Deterministic output means
no run-to-run variance unlike v1 (which ranged 3.2%–4.1%).
"""
...
class DDPM:
    def __init__(self, T_diff=100, t_eval=35, n_steps=12, step_size=0.45,
...
def build_denoiser() -> DDPM:
    return DDPM(T_diff=100, t_eval=35, n_steps=12, step_size=0.45,
...
if __name__ == "__main__":
    ...
    print(f"Training fast score-guidance denoiser for {TIME_BUDGET}s on {denoiser.device} ...")
    ...
    _log_results(results, "score guidance fast det n_steps=12 step_size=0.45", git_hash)
