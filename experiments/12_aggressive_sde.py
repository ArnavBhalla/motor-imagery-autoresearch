"""
Experiment 12: Very high SDEdit noise level (sde_t0=0.90).

Hypothesis: at t0=0.90 the decoded trajectory is almost completely replaced by
Gaussian noise before the reverse chain. The model must reconstruct primarily
from its learned manifold rather than from the corrupted input. The output
diverges substantially from what Gaussian smoothing produces (which still
resembles the input), so the not_trivial guardrail may pass.
Risk: destroying too much input signal could hurt RMSE.
"""
...
class DDPM:
    def __init__(self, T_diff=100, sde_t0=0.90, n_infer=40,
...
def build_denoiser() -> DDPM:
    return DDPM(T_diff=100, sde_t0=0.90, n_infer=40, channels=64, depth=4, lr=2e-4, batch_size=256)
...
if __name__ == "__main__":
    ...
    print(f"Training aggressive-SDEdit denoiser for {TIME_BUDGET}s on {denoiser.device} ...")
    ...
    _log_results(results, "aggressive SDEdit sde_t0=0.90 n_infer=40", git_hash)
