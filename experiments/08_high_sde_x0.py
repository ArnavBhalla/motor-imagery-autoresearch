"""
Experiment 08: Combine best ideas — high SDEdit + x0-prediction + cosine schedule.

Hypothesis: if sde_t0=0.65 (exp 04) and x0-prediction (exp 02) and cosine
schedule (exp 03) each help independently, combining them should yield a
larger gain. This is the first "combination" experiment.
"""
...
class DDPM:
    """x0-prediction + cosine schedule + high SDEdit."""
    ...
    def __call__(self, decoded_traj):
        ...
        with torch.no_grad():
            for i in range(self.n_infer):
                ...
                # x0-prediction DDIM
                pred_x0 = self.model(x_t, t_c.float()).clamp(-3,3)
                pred_eps = (x_t - ab_c.sqrt()*pred_x0) / (1-ab_c).sqrt()
                x_t = ab_p.sqrt()*pred_x0 + (1-ab_p).sqrt()*pred_eps
...
if __name__ == "__main__":
    ...
    _log_results(results, "combination: x0-pred + cosine + sde_t0=0.65", git_hash)
