"""
Experiment 24: Supervised CNN (bigger model) — channels=128, depth=6.

Same noise2clean training as exp 23, but larger capacity. With 600s budget
and only 1600 training pairs, this risks overfitting — but the residual
skip connection limits damage and the noise pairs are deterministic.
"""
...
def build_denoiser() -> SupervisedDenoiser:
    return SupervisedDenoiser(channels=128, depth=6, lr=5e-4, batch_size=64)
...
if __name__ == "__main__":
    ...
    print(f"Training big supervised CNN for {TIME_BUDGET}s on {denoiser.device} ...")
    ...
    _log_results(results, "supervised CNN big channels=128 depth=6", git_hash)
