"""
Experiment 28: Supervised Transformer denoiser (1D sequence attention).

Replaces the CNN with a Transformer encoder over the 100-timestep trajectory.
Each timestep is one token. Self-attention gives global receptive field from
layer 1, letting the model use full-sequence context (start of motion informs
end of motion) — which CNNs can only achieve with many stacked layers.

Architecture: linear embed → learned pos encoding → N × (MHSA + FFN) → linear out
Residual: model predicts the correction; input + correction = output.
"""
...
class SupervisedDenoiser:
    ...
    def train(self, trajectories, time_budget=TIME_BUDGET, verbose=True):
        ...
        Xt = torch.from_numpy(X_n).to(self.device)   # (N, T, dim) — no transpose needed
    ...
    def __call__(self, decoded_traj):
        ...
        xt = torch.from_numpy(x).float().unsqueeze(0).to(self.device)  # (1, T, dim)
        self.model.eval()
        with torch.no_grad():
            out = self.model(xt)
...
def build_denoiser() -> SupervisedDenoiser:
    return SupervisedDenoiser(n_aug=4, d_model=64, n_heads=4, n_layers=4)
...
if __name__ == "__main__":
    ...
    _log_results(results, "transformer denoiser d=64 heads=4 layers=4 n_aug=4", git_hash)
