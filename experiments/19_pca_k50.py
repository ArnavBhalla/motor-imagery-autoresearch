"""
Experiment 19: PCA manifold projection, K=50 components.

Looser projection than K=30 — retains more local motion detail.
For GRAB data with 9200 diverse clips, K=50 may capture enough
fine structure to meaningfully correct imagery trajectories without
over-smoothing.
"""
...
def build_denoiser() -> PCADenoiser:
    return PCADenoiser(n_components=50)
...
if __name__ == "__main__":
    ...
    print(f"Training PCA denoiser (K=50) ...")
    ...
    _log_results(results, "PCA manifold projection K=50", git_hash)
