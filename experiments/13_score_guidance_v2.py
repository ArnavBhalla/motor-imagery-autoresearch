"""
Experiment 13: Score guidance v2 — deterministic + more aggressive.

Score guidance v1 (3.20%) is the best result but still near-Gaussian.
Two changes:
  1. Deterministic: use a fixed noise seed per trajectory so each call
     produces the same output (no stochastic variance diluting the result).
  2. More aggressive: step_size=0.35, n_steps=40 to push further toward
     the manifold and diverge more from Gaussian behavior.
"""
...
class DDPM:
    """
    Score guidance v2: deterministic fixed-seed noise + larger step_size.
    Average predicted x0 over K noise draws for a low-variance estimate,
    then take a larger step toward it.
    """
    def __init__(self, T_diff=100, t_eval=35, n_steps=40, step_size=0.35, n_avg=4,
                 channels=64, depth=4, lr=2e-4, batch_size=256, t_dim=32, device=None):
        self.T_diff = T_diff; self.t_eval = t_eval
        self.n_steps = n_steps; self.step_size = step_size; self.n_avg = n_avg
    ...
    def __call__(self, decoded_traj):
        ...
        with torch.no_grad():
            for step_i in range(self.n_steps):
                # Average predicted x0 over n_avg noise draws for lower variance
                pred_x0_sum = torch.zeros_like(x_cur)
                for k in range(self.n_avg):
                    g.manual_seed(step_i * self.n_avg + k)
                    eps_in = torch.randn(x_cur.shape, generator=g, device=self.device)
                    ...
                    pred_x0_sum += pred_x0
                pred_x0_avg = pred_x0_sum / self.n_avg
                x_cur = x_cur + self.step_size * (pred_x0_avg - x_cur)
        return x_cur.squeeze(0).cpu().numpy() * self._std[0] + self._mean[0]


def build_denoiser() -> DDPM:
    return DDPM(T_diff=100, t_eval=35, n_steps=40, step_size=0.35, n_avg=4,
...
if __name__ == "__main__":
    ...
    print(f"Training score-guidance-v2 denoiser for {TIME_BUDGET}s on {denoiser.device} ...")
    ...
    _log_results(results, "score guidance v2 det+avg n_steps=40 step_size=0.35", git_hash)
