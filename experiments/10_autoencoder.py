"""
Experiment 10: Autoencoder manifold projection.

Hypothesis: the not_trivial guardrail fails because diffusion+SDEdit learns
to smooth rather than to project. A tight-bottleneck AE forces decoded
trajectories onto a learned nonlinear manifold — the output is structurally
impossible to replicate with a Gaussian filter, so it either beats or clearly
diverges from Gaussian smoothing.

Architecture: Conv1d encoder → 32D bottleneck → Conv1d decoder.
Inference: AE(decoded_traj) — no stochastic sampling, pure manifold projection.
"""
...
class TrajectoryAE(nn.Module):
    def __init__(self, traj_dim=TRAJ_DIM, seq_len=SEQ_LEN, bottleneck=32, channels=64):
        super().__init__()
        # Encoder: (B, traj_dim, T) → (B, bottleneck)
        ...
        # Decoder: (B, bottleneck) → (B, traj_dim, T)
    ...
    def encode(self, x):
        # x: (B, T, traj_dim)
        h = self.enc(x.permute(0, 2, 1))        # (B, C, T)
        return self.enc_proj(h.flatten(1))       # (B, bottleneck)

    def decode(self, z):
        h = self.dec_proj(z).view(-1, self.channels, self.seq_len)
        return self.dec(h).permute(0, 2, 1)      # (B, T, traj_dim)
...
class AEDenoiser:
    def __init__(self, bottleneck=32, channels=64, lr=1e-3, batch_size=256, device=None):
...
def build_denoiser() -> AEDenoiser:
    return AEDenoiser(bottleneck=32, channels=64, lr=1e-3, batch_size=256)
...
if __name__ == "__main__":
    ...
    print(f"Training AE denoiser for {TIME_BUDGET}s on {denoiser.device} ...")
    ...
    _log_results(results, "autoencoder manifold projection bottleneck=32", git_hash)
