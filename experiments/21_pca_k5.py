"""
Experiment 21: PCA manifold projection, K=5 components (synthetic data).

Synthetic sinusoidal trajectories have ~5 dominant modes: amplitude/frequency/
phase per axis, plus mean. K=5 is the most aggressive projection — it forces
the decoded imagery trajectory onto the exact sinusoidal subspace, discarding
all non-sinusoidal noise components. Expected to be much more effective than
score guidance because the manifold is correctly specified.
"""
...
def build_denoiser() -> PCADenoiser:
    return PCADenoiser(n_components=5)
...
if __name__ == "__main__":
    ...
    print(f"Training PCA denoiser (K=5) ...")
    ...
    _log_results(results, "PCA manifold projection K=5 synthetic", git_hash)
