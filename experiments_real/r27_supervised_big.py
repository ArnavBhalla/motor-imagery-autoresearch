"""
Experiment r27: Supervised CNN denoiser on BCI IV Dataset 4 (real ECoG).

Same ResNet CNN architecture as synthetic exp 27 (ch=128, d=6).
Trains on (decoder_output, true_DataGlove) pairs from real ECoG recordings.
No n_aug — one recording per window, but 50% overlap gives ~2400 windows.
"""
...
from prepare_bciiv4 import (SEQ_LEN, TRAJ_DIM, TIME_BUDGET, FIXED_SEED,
...
class SupervisedDenoiser:
    def __init__(self, channels=128, depth=6, lr=5e-4, batch_size=64, device=None):
    ...
    def _make_pairs(self, decoded_trajs, true_trajs):
        # decoded_trajs are already decoder outputs (identity decoder in real pipeline)
    ...
    def train(self, decoded_trajs, true_trajs, decoder,
        ...
        X, y = self._make_pairs(decoded_trajs, true_trajs)
...
def build_denoiser():
    return SupervisedDenoiser(channels=128, depth=6)


if __name__ == "__main__":
    desc = "real CNN big ch=128 d=6 ECoG-HG"
    ...
    print(f"Training CNN denoiser for {TIME_BUDGET}s on {denoiser.device} ...")
